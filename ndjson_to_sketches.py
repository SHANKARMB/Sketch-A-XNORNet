...
rootDir = '/home/prime/ProjectWork/training/dataset/22gb/'
...
ndjson_list = ['airplane', 'hot air balloon', 'snake', 'pineapple', 'butterfly',
               'knife', 'wine bottle', 'apple', 'hamburger', 'scissors']
for ndj_file in ndjson_list:
    folder = os.path.join(final_dir, ndj_file)
    print('folder name', folder)
    os.makedirs(folder, exist_ok=True)
    file_name = 0
    count = 0
    print('gen random num')
    x = random.sample(range(100000), 11000)
    print('processing ', ndj_file)
    with jsonlines.open(os.path.join(current_dataset, ndj_file+'.ndjson'), mode='r') as reader:
        for obj in reader:
            if count in x:
